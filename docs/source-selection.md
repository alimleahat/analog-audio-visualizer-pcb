...
The working `audiovisualiser` directory, the Week 4 submission, and `AudioVisualiserFiles` contain byte-identical schematic and PCB files. The working directory also contains the matching KiCad project settings, which are included so the saved design rules travel with the schematic and board.

The seven Gerber files match the Week 4 production archive byte-for-byte. Plated and non-plated drill files, the Gerber job file, the BOM, and component positions come from the same working project. Manufacturing files are historical exports; they were not regenerated during portfolio preparation.
...
The source archive remains untouched. Project files, manufacturing exports, and selected standalone images were copied; assembly photos were extracted from the existing PDF. Documentation and new KiCad check reports were added for this repository.
